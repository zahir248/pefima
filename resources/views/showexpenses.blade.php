...
<form method="GET" action="{{ route('showexpenses', ['id' => $category->id]) }}" class="mb-6">
    <div class="flex items-center space-x-4">
        <div class="w-1/4">
            ...
            <select name="month" id="month" class="mt-1 block w-full py-2 px-5 border border-gray-300 rounded-md">
        ...
        <div class="w-1/4" style="margin-left: 10px">
            ...
            <select name="year" id="year" class="mt-1 block w-full py-2 px-5 border border-gray-300 rounded-md">
        ...
        <div class="w-1/4" style="margin-top: 25px; margin-left: 10px;">
            <button type="submit" class="btn btn-primary py-2 px-4">Filter</button>
                        ...
                        <a href="{{ route('downloadpdf', $category->id) }}" class="bg-blue-500 hover:bg bg-info text-white font-bold py-2 px-4 rounded" style="margin-top: 25px ;margin-left: 590px">
                            Download PDF
                        </a>
    ...
    @if(request('month') && request('year'))
        <p class="text-xl font-semibold text-gray-800">
            Total Amount for {{ DateTime::createFromFormat('!m', $month)->format('F') }} {{ $year }}: RM {{ number_format($filteredTotalAmount, 2) }}
        </p>
    @else
        <p class="text-xl font-semibold text-gray-800">
